...
export async function loadAgentState(sessionId: string): Promise<AgentSetupState | null> {
  ...
  const { data, error } = await supabase.functions.invoke<{ state: null | {
    stage: number; messages: StoredChatMessage[]; blocks: StoredBlock[]; pending_blocks: StoredBlock[]
    goal: string; reason: string; obstacle: string; openai_response_id?: string; goal_card:GoalCard; goal_response_id?:string; generated_plan?:GeneratedPlanItem[]; plan_response_id?:string
