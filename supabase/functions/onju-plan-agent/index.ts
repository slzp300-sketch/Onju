...
type DayBounds={days?:number[];wake?:string;bedtime?:string}
...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:corsHeaders});try{
 ...
 const body=await req.json();const goal=body.goalCard??{};const schedule:Array<TimeBlock>=Array.isArray(body.scheduleBlocks)?body.scheduleBlocks:[];const dayBounds:Array<DayBounds>=Array.isArray(body.dayBounds)?body.dayBounds:[]
...
- 각 요일의 confirmed_day_bounds에 있는 기상 이후, 취침 이전에만 배치한다. 생활시간이 없는 요일에는 계획을 만들지 않는다.
...
- 목표 카드에 없는 성과나 수치를 새로 만들지 않는다.`
 ...
 for(const item of (Array.isArray(parsed.items)?parsed.items:[]) as PlanItem[]){
  const boundary=dayBounds.find(saved=>saved.days?.includes(item.dayIndex));const wake=boundary?.wake?toMinutes(boundary.wake):0;const bedtime=boundary?.bedtime?toMinutes(boundary.bedtime):0
  const withinDay=Boolean(boundary)&&toMinutes(item.start)>=wake&&(bedtime<=wake||toMinutes(item.end)<=bedtime)
  const validRange=item.start<item.end&&withinDay
