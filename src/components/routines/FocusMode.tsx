...
export default function FocusMode({ routine, onClose }: FocusModeProps) {
  ...
  const finish = useCallback((actual: number) => {
    ...
    if (!isCompleted(routine.id, today())) toggleRoutineLog(routine.id, today());
    setActualSecs(actual);
    setPhase('done');
    confetti({ particleCount: 100, spread: 70, origin: { y: 0.3 }, colors: [color, '#f59e0b', '#fff'] });
  }, [routine.id, color, isCompleted, toggleRoutineLog]);
