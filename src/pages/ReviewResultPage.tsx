  ...
  if (!review) {
    return (
      <div className="flex flex-col items-center justify-center min-h-dvh gap-4 px-6">
        <p className="text-label-alt text-sm">해당 주차의 리뷰를 찾을 수 없어요.</p>
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
      ...
      <div className="px-4 pt-4 flex items-center gap-2">
        ...
        <div>
          <h1 className="text-base font-bold text-label-strong">
      ...
      <div className="px-4 grid grid-cols-2 gap-3">
        <Card className="!p-4 text-center">
          <p className="text-xs text-primary font-medium mb-1">개인 루틴</p>
          <p className="text-3xl font-bold text-primary">{review.personalRate}%</p>
        </Card>
        <Card className="!p-4 text-center">
      ...
      {/* 목표 달성 */}
      <Card className="mx-4">
        <p className="text-xs font-semibold text-label-alt mb-2">주간 목표</p>
      ...
      {mood && (
        <Card className="mx-4">
          <p className="text-xs font-semibold text-label-alt mb-2">이번 주 느낌</p>
          <p className="text-lg">{mood.emoji} <span className="text-sm text-label">{mood.label}</span></p>
        </Card>
      )}
      ...
      {Object.keys(review.goalRatings).length > 0 && (
        <Card className="mx-4">
          <p className="text-xs font-semibold text-label-alt mb-3">목표 별점</p>
          <div className="flex flex-col gap-2">
            {Object.entries(review.goalRatings).map(([goalId, rating]) => (
              <div key={goalId} className="flex items-center gap-2">
                <span className="text-xs text-label-alt flex-1 truncate">{goalId}</span>
      ...
      {review.comment && (
        <Card className="mx-4">
          <p className="text-xs font-semibold text-label-alt mb-2">이번 주 소감</p>
          <p className="text-sm text-label leading-relaxed">"{review.comment}"</p>
        </Card>
      )}
      ...
      {review.intention && (
        <Card className="mx-4">
          <p className="text-xs font-semibold text-label-alt mb-2">다음 주 집중</p>
          <p className="text-sm font-medium text-primary">→ {review.intention}</p>
        </Card>
      )}
      ...
      {review.routineChanges.filter(c => c.action !== 'keep').length > 0 && (
        <Card className="mx-4">
          <p className="text-xs font-semibold text-label-alt mb-3">루틴 조정 내역</p>
          <div className="flex flex-col gap-2">
